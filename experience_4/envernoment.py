...
class env():
    ...
    def step(self, action):
        ...
        reward = (start_dist - self.dist) * 0.001
    ...
    def render(self):
        ...
        self.canvas.delete(self.line_3)
        ...
        self.line_2 = self.canvas.create_line(self.zero_x + self.a1 * m.cos(-self.q1),
                                              ...
                                              self.zero_y + self.a1 * m.sin(-self.q1) + self.a2 * m.sin(
                                                  -(self.q1 + self.q2)),
                                              width=5, fill='green')
        self.line_3 = self.canvas.create_line(self.zero_x + self.a1 * m.cos(-self.q1) + self.a2 * m.cos(
                                                  -(self.q1 + self.q2)),
                                              self.zero_y + self.a1 * m.sin(-self.q1) + self.a2 * m.sin(
                                                  -(self.q1 + self.q2)),
    ...
    def create_random_ball(self):
        if len(self.target_position) == 0:
            ...
            q3 = m.radians(r.uniform(-90, 90))
            self.ball_coord_x = self.zero_x + self.a1 * m.cos(-q1) + self.a2 * m.cos(-q1 - q2) +\
                                self.a3 * m.cos(-q1 - q2 - q3)
            self.ball_coord_y = self.zero_y + self.a1 * m.sin(-q1) + self.a2 * m.sin(-q1 - q2) +\
                                self.a3 * m.sin(-q1 - q2 - q3)
        else:
            self.ball_coord_x = self.zero_x + self.target_position[0]
            self.ball_coord_y = self.zero_y + self.target_position[1]
    ...
    def create_random_q(self):
        ...
        if len(self.sours_q) == 0:
            ...
            self.q3 = r.uniform(-m.pi/2, m.pi/2)
        else:
    ...
    def get_obs(self):
        ...
        self.end_x = self.zero_x + self.a1 * m.cos(-self.q1) + self.a2 * m.cos(-self.q1 - self.q2) +\
                     self.a3 * m.cos(-self.q1 - self.q2 - self.q3)
        self.end_y = self.zero_y + self.a1 * m.sin(-self.q1) + self.a2 * m.sin(-self.q1 - self.q2) +\
                     self.a3 * m.sin(-self.q1 - self.q2 - self.q3)
        ...
        result = np.array(
            [self.dist / 600,
             (self.q1 + m.pi) / (2 * m.pi), (self.q2 + m.pi / 2) / m.pi, (self.q3 + m.pi / 2) / m.pi, ((self.end_x - self.zero_x) + 300) / 600,
